...
class Calc {
  ...
  static _isEqualInput(input) {
    return parseInt(input) === Calc.OP.EQ;
  }

  static _isResetInput(input) {
    return parseInt(input) === Calc.OP.RES;
  }

  static _isOperationInput(input) {
    let operation = Calc.OP.input_to_operation_map[input];
    let input_operation = Calc.OP[operation];
    if (input_operation !== undefined) {
      return (input_operation !== Calc.OP.EQ) && (input_operation !== Calc.OP.RES);
    }
    return false;
  }
  ...
  _handleEqual() {
    ...
    let resultFOp1ST = Calc._getOperationResult(this.__FIRST, this.__OP1, resultSOp2T);
    switch (this.__STATE) {
      case Calc.STATE.TRAILING:
        this.__FIRST = resultFOp1ST;
        break;
  ...
  _handleOperationInput(input_operation) {
    ...
    switch (this.__STATE) {
      ...
      case Calc.STATE.TRANSITION_FROM_TRANSITION:
        if (!Calc._isPlusOrMinusOperation(input_operation) && Calc._isPlusOrMinusOperation(this.__OP1)) {
        ...
        break;
      case Calc.STATE.TRAILING:
        if (Calc._isPlusOrMinusOperation(input_operation)) {
          // simple operation case: move back to TRANSITION
          this.__FIRST = resultFOp1ST;
          this.__OP1 = input_operation;
          this.__SECOND = resultFOp1ST;
          this.__DISPLAY = Calc.DISP.FIRST;
          this.__STATE = Calc.STATE.TRANSITION;
        } else {
